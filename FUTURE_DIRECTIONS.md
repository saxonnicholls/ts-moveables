...
**Head-to-head, done.** `make bench-compare` now runs the same harness against moodycamel's `ReaderWriterQueue` (SPSC) and `ConcurrentQueue` (MPMC) — the field's reference implementations, fetched on demand and never committed (the library stays dependency-free). The honest result: **moodycamel wins, decisively on SPSC (~367 vs ~28 Mops/s) and clearly on MPMC (~6.8 vs ~4.2 Mops/s)** on the Intel box. Two measured findings behind it: ~11 ns of our ~35 ns SPSC op is the quiescent-move-check (the ring runs ~41 Mops/s with it neutralised) — the price of moveability that moodycamel's immovable queues don't pay — and the rest is design (their block-based amortisation vs our per-op slot atomics). Our batched paths (`push_n`, `publish_n`) stay competitive because they amortise the same way. This is exactly what the non-goals predicted, now with receipts, and it retires the "performance claims unproven" gap honestly: we do not beat the specialist queues on raw single-op throughput, and the docs say so plainly. The comparison also motivated a shipped refinement: `circular_buffer<T, N, Checked>` now carries a compile-time **safety-vs-performance switch**. `Checked = true` (default) keeps the quiescent-move-check; `Checked = false` compiles it out for a proven hot path. It recovers the ~6 ns/op the check costs — but the head-to-head proves the check was never the real gap: even unchecked we stay ~13× behind moodycamel, because that gap is architectural (block-based amortisation vs our per-op slot atomics), not the move-check. So the switch is a genuine developer choice, not a moodycamel-killer; anyone who needs ~450 Mops/s single-op SPSC should reach for moodycamel, exactly as the non-goals say.
