...
- **Leverage [base-encode-decode](https://github.com/saxonnicholls/base-encode-decode)** — as an *optional adapter*, not a core dependency. The WebSocket accept key needs base64 of one 20-byte digest, which is ~35 lines and is kept local so the drop-in single header stays genuinely dependency-free. Where the library earns its place is **HTTP Basic auth**: real base64 of arbitrary credentials, with the padding and validation edge cases an encoding library should own. Add it then, in the same tier as `tls/openssl.hpp`.

**Packaging: genuinely one file.** `http/server.hpp` is a single header that includes `event/loop.hpp` and `moveable_signal.hpp` from the same directory. For true httplib-style drop-in, `scripts/amalgamate.sh` emits `single_include/ts_http_server.hpp` — one self-contained file, no include path, no build system, nothing to link but pthreads.
...
| **2** | ✅ **Shipped, both halves.** `tls/openssl.hpp` (memory-BIO) and `tls/mbedtls.hpp` — mbedTLS chosen deliberately because it has *no* BIO abstraction (the library calls your callbacks), and the interface fit it **unchanged**: `http/server.hpp` needed not one line. The same test bodies run against both backends. Streaming responses (`res.stream()`) shipped alongside. Ships with **streaming responses** (`res.stream()`), since both touch the write path and a TLS file server that buffers whole files in memory is not a file server | HTTPS end-to-end, ALPN reported, two backends passing the same tests |
| **3** | ✅ **WebSocket shipped** (`websocket.hpp` + `http/websocket_deflate.hpp`): handshake, framing, masking, fragmentation, close/ping/pong, UTF-8 validation as a protocol-delegate swap, and `permessage-deflate` as an extension delegate | ✅ **Autobahn clean: 517 of 517 cases, 0 failures, 0 unimplemented** (510 OK, 4 non-strict, 3 informational). `make autobahn` |
| **4** | ✅ **HTTP/2 shipped** (`http/http2.hpp`): framing, HPACK with compile-time Huffman tables, stream state machine, bidirectional flow control, h2 over ALPN and h2c prior knowledge, and the abuse limits from the start. The stream id threaded through the interfaces since phase 1 needed no change | ✅ **h2spec: 147 of 147, 0 failed** (`make h2spec`) |
...
**What the second TLS backend found.** Exactly what a second implementation is for - not a bug in the new code, but one in ours that a single backend could never have exposed. Both engines carefully queue a TLS alert into `wire_out` before failing (`handshake_failure`, `bad_certificate`), and the reactor threw every one of them away: a fatal `wire_in` went straight to `close_now()`, which closes the descriptor without flushing. Peers got a bare FIN and no reason. The same audit showed `transport->shutdown()` was never called at all, so a clean close never sent `close_notify` either. The close path now consults the transport in both cases.
