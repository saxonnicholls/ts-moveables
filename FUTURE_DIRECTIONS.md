...
| **2** | ✅ **Shipped, both halves.** `tls_openssl.hpp` (memory-BIO) and `tls_mbedtls.hpp` — mbedTLS chosen deliberately because it has *no* BIO abstraction (the library calls your callbacks), and the interface fit it **unchanged**: `http_server.hpp` needed not one line. The same test bodies run against both backends. Streaming responses (`res.stream()`) shipped alongside. Ships with **streaming responses** (`res.stream()`), since both touch the write path and a TLS file server that buffers whole files in memory is not a file server | HTTPS end-to-end, ALPN reported, two backends passing the same tests |
