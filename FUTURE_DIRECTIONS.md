...
| **2** | TLS delegates: OpenSSL memory-BIO engine first, then a second backend to prove the interface. Ships with **streaming responses** (`res.stream()`), since both touch the write path and a TLS file server that buffers whole files in memory is not a file server | HTTPS end-to-end, ALPN reported, two backends passing the same tests |
