...
build/tests: tests/tests.cpp $(HEADERS) | build
	$(CXX) $(CXXFLAGS) -pthread tests/tests.cpp -o $@

build/tests_tsan: tests/tests.cpp $(HEADERS) | build
	$(CXX) $(CXXFLAGS) -pthread -fsanitize=thread tests/tests.cpp -o $@

build/tests_asan: tests/tests.cpp $(HEADERS) | build
	$(CXX) $(CXXFLAGS) -pthread -fsanitize=address,undefined tests/tests.cpp -o $@

build/demo: TSMoveables/main.cpp $(HEADERS) | build
	$(CXX) $(CXXFLAGS) -pthread TSMoveables/main.cpp -o $@
